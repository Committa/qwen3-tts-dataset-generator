...
poetry run gen-dataset --step publish --wavs-per-dir 9000  # override wavs_per_dir for the archive layout
...
poetry run merge-datasets           # merge the two most recent archives (output/genNNN) into the next gen folder; secondary archive re-indexed after the primary and added entirely to train, report.json merged, concatenated metadata.csv (train+val) emitted, transcript overlap is refused
...
- `src/common.py` defines `Config`/`Paths` dataclasses, `PROJECT_ROOT`, and all shared helpers (logging, checkpoint, OOM detection). All other modules import from here. The `speaker` field on `Config` is the voice identity for both modes (preset name for `custom_voice`, custom voice name under `inputs/voices/` for `base`); `x_vector_only_mode` is a flattened top-level field used only in `base` mode. The `Paths` dataclass holds all pipeline paths: only `input_sentences` and `test_sentences` are configurable in `config.yaml` (top-level keys); all other paths (`raw_wav`, `accepted_wav`, `normalized_wav`, `rejected`, manifests, `report`, `checkpoint`, `log_file`, `prompt_cache`) are fixed defaults and resolved relative to `PROJECT_ROOT`.
- Workspace layout (post-validate onwards): `raw_wav/` is the sorgente grezza grezza, kept intact because `validate` copies (never moves) clips into `accepted_wav/` (accepted) or `rejected/` (rejected). `accepted_wav/` is left untouched by every downstream step: `normalize` writes the normalized copy to `normalized_wav/` (resumable via filesystem existence — a clip whose destination file already exists is skipped; delete the directory to force a full re-normalize), and `publish` (`common.archive_generation`) **copies** from `normalized_wav/` to `output/gen{NNN}/wavs/` without emptying the workspace. The archive is regenerated deterministically: `wavs/`, `metadata_*.csv` and `report.json` are wiped before rewriting, while manual artifacts (e.g. `README.md`, `LICENSE`) in the gen directory are preserved. `Config.wavs_per_dir` (default 9000, override via `--step publish --wavs-per-dir N` or `0` for flat) caps wav files per archive directory: >0 spreads them over `wavs/<first>-<last>/` subdirectories and rewrites the metadata paths accordingly, because the Hugging Face Hub rejects commits with >10000 files in a single directory. Net effect: after publish, the workspace state (`raw_wav` + `accepted_wav` + `normalized_wav` + `rejected`) is still complete, so any step from `validate` onward can be re-run without losing clips.
...
- Config fields (in `common.py` `Config`, gated by `phoneme_check`): `phoneme_model`, `phoneme_device`, `phoneme_batch_size` (wav2vec2 CTC is not thread-safe; batching is used instead of workers), `phoneme_cleanup_every_n_batches` (default 10 — cadence at which the recognize loop frees the PyTorch CUDA allocator pool via `common.cleanup_gpu`; without this the reserved pool grows monotonically across hundreds of short forwards until OOM, since the caching allocator never returns memory to the driver on its own), `phoneme_threshold` (default 0.30 — phoneme recognition is noisier than word ASR), `phoneme_word_report` (default true) and `phoneme_word_top_n` (default 20) for the per-word diagnostic, `phoneme_report_min_occurrences` (default 3 — minimum occurrences for a word to appear in the "worst words" report; filters out one-off artefacts). In a full run the step is skipped when `phoneme_check: false`; an explicit `--step pronunciation` always runs it.
...
- `--only-rejected` (with `--step pronunciation`): re-score pronunciation-rejected clips still sitting in `rejected/`. Step-SPECIFIC (unlike generate/validate --only-rejected which are step-agnostic): only sidecars carrying `ref_phonemes`/`hyp_phonemes` (pronunciation-sidecars) are eligible; validate-rejected sidecars (transcription/wer only) are skipped — there is no point scoring PER on a clip whose WORDS are wrong. Wavs are read from `rejected/` in place (NOT from `accepted_wav/`); the `done` checkpoint is NOT used as a filter (the user is explicitly asking to re-evaluate previously-rejected audio, e.g. after raising `phoneme_threshold`). Accept moves the wav back to `accepted_wav/` and removes the sidecar (clip no longer rejected); reject just refreshes the sidecar PER in place (wav stays in `rejected/`). The checkpoint is updated at the end of each run (MERGE in both modes, matching `generate`/`validate` — `done` is a monotonic set).
- **Regeneration cycle** (`generate --only-rejected` + plain `pronunciation`): `generate --only-rejected` invalidates the pronunciation `done` checkpoint for the regenerated indices (P2 logic in `generate.py`): the new audio has not been pronunciation-checked yet, so removing the index from `done` is exactly the right trigger — the next standard `pronunciation` run picks up the regenerated clips via normal resumability (`accepted_wav/ - done`). No special flag, no extra file. The three checkpoints (`generate`, `validate`, `pronunciation`) all share the same monotonic `done` contract now: MERGE, never REPLACE. `review-rejected` does NOT touch any checkpoint — manual accept is a human judgment on audio quality, not a PER pass.
