...
- **Regenerated-only path** (no flag, after `generate --only-rejected`): `generate --only-rejected` writes `workspace/.regenerated.json` containing the indices it just regenerated. The next `pronunciation` run (no flag) consumes that file **one-shot** (deleted after read) and processes ONLY the regenerated indices intersected with `accepted_wav/`. Resumability (`done`) is bypassed for these — the new audio has no PER score yet and MUST be re-scored. This design is robust to operations that mutate `done` between regen and pronunciation (e.g. `review-rejected` manual-accept), which would otherwise re-populate `done` with the manually-accepted clips and confuse the P2 design that used to live here. The file path is `cfg.paths.regenerated = workspace/.regenerated.json` (see `common.Paths`).
