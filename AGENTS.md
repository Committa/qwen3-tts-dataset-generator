...
poetry run gen-dataset --step pronunciation --reset-done  # delete pronunciation_checkpoint and re-check every clip in accepted_wav/ (skip resumability)
...
- `--only-rejected` (with `--step pronunciation`): re-score pronunciation-rejected clips still sitting in `rejected/`. Step-SPECIFIC (unlike generate/validate --only-rejected which are step-agnostic): only sidecars carrying `ref_phonemes`/`hyp_phonemes` (pronunciation-sidecars) are eligible; validate-rejected sidecars (transcription/wer only) are skipped — there is no point scoring PER on a clip whose WORDS are wrong. Wavs are read from `rejected/` in place (NOT from `accepted_wav/`); the `done` checkpoint is NOT used as a filter (the user is explicitly asking to re-evaluate previously-rejected audio, e.g. after raising `phoneme_threshold`). Accept moves the wav back to `accepted_wav/` and removes the sidecar (clip no longer rejected); reject just refreshes the sidecar PER in place (wav stays in `rejected/`). The checkpoint is updated at the end of each run (merge on `--only-rejected`, replace on a full pass). Manually accepted clips via `review-rejected` (a) and re-rejected clips (b) are also tracked in the checkpoint. NOTE: `generate --only-rejected` invalidates the pronunciation `done` checkpoint for the regenerated indices (the new audio has not been pronunciation-checked yet), so a plain `--step pronunciation` after the regeneration cycle automatically re-scores the new clips via resumability (skips `done`, processes the rest) — this is the P2 design documented in `generate.py`. If the user does not want resumability (e.g. they want a full re-evaluation of every clip in `accepted_wav/` after several regen/validate cycles, ignoring the P2 bookkeeping), pass `--reset-done` to delete `workspace/.pronunciation_checkpoint.json` before the run.
