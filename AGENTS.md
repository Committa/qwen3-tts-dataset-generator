...
- `--only-rejected` (with `--step pronunciation`): re-score pronunciation-rejected clips still sitting in `rejected/`. Step-SPECIFIC (unlike generate/validate --only-rejected which are step-agnostic): only sidecars carrying `ref_phonemes`/`hyp_phonemes` (pronunciation-sidecars) are eligible; validate-rejected sidecars (transcription/wer only) are skipped — there is no point scoring PER on a clip whose WORDS are wrong. Wavs are read from `rejected/` in place (NOT from `accepted_wav/`); the `done` checkpoint is NOT used as a filter (the user is explicitly asking to re-evaluate previously-rejected audio, e.g. after raising `phoneme_threshold`). Accept moves the wav back to `accepted_wav/` and removes the sidecar (clip no longer rejected); reject just refreshes the sidecar PER in place (wav stays in `rejected/`). The checkpoint is updated at the end of each run (merge on `--only-rejected`, replace on a full pass). Manually accepted clips via `review-rejected` (a) and re-rejected clips (b) are also tracked in the checkpoint.
- **Regenerated-only path** (no flag, after `generate --only-rejected`): `generate --only-rejected` writes `workspace/.regenerated.json` containing the indices it just regenerated. The next `pronunciation` run (no flag) reads the file and processes ONLY the regenerated indices intersected with `accepted_wav/`, filtering out any already in `done`. The manifest is **deleted only at the end of a successful `run_pronunciation`** invocation: a Ctrl+C (or any other exception) mid-check leaves the file in place so the next `pronunciation` run can pick up where the previous one stopped. Resumability (`done`) is bypassed for these — the new audio has no PER score yet and MUST be re-scored. This design is robust to operations that mutate `done` between regen and pronunciation (e.g. `review-rejected` manual-accept), which would otherwise re-populate `done` with the manually-accepted clips and confuse the P2 design that used to live here. The file path is `cfg.paths.regenerated = workspace/.regenerated.json` (see `common.Paths`).
