...
poetry run gen-dataset --step pronunciation --only-rejected  # re-score only clips rejected by pronunciation (reads workspace/.pronunciation_checkpoint.json)
...
- `--only-rejected` (with `--step pronunciation`): re-score only the clips that were not in the previous pronunciation checkpoint (`workspace/.pronunciation_checkpoint.json`). Mirrors the regenerate cycle for the TTS step: after `generate --only-rejected` and `validate --only-rejected` move the regenerated clips back into `accepted_wav/`, `pronunciation --only-rejected` skips the clips already accepted and re-scores only the new ones. The checkpoint is updated at the end of each run (merge on `--only-rejected`, replace on a full pass). Manually accepted clips via `review-rejected` (a) and re-rejected clips (b) are also tracked in the checkpoint.
