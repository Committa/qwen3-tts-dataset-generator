...
- `--only-rejected` (with `--step pronunciation`): re-score only the clips currently in `rejected/*.json` that are also in `accepted_wav/` and not already in the pronunciation checkpoint. Same `done`-set semantic as generate/validate (uniform across all three steps: `done` = "fully processed, do not redo"). Source of "what to consider" is the `rejected/` directory — step-agnostic, no marker check on the sidecar fields. The checkpoint is updated at the end of each run (merge on `--only-rejected`, replace on a full pass). Manually accepted clips via `review-rejected` (a) and re-rejected clips (b) are also tracked in the checkpoint.
